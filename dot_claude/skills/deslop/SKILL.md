...
1. **Rules and documentation conformance**
   - Are we following `CLAUDE.md`, skill files, and any design docs?
   - Did we drift from documented repo patterns or ownership boundaries?
   - Are we violating scraping policy, git policy, or DB conventions from CLAUDE.md?
...
2. **Type safety and source of truth**
   ...
   - Prefer compile-time guarantees over runtime defensive programming inside typed repo-owned code. Validate and parse only at untrusted boundaries. Once data has crossed a validator-owned boundary or is carried by an inferred repo-owned type, trust it downstream and do not re-parse it.
...
- `CLAUDE.md` (repo root)
- Any relevant skill files from `.claude/skills/` for the changed areas
- `docs/patterns.md`
- Any design doc directly relevant to the changed area
- The changed files and enough nearby context to review them properly

If any of these files don't exist, skip them — only include what's actually present.
...
4. Wait for all review agents to return.
   - While they run, start with `make lint` to prime yourself on the highest priority deltas, newly introduced hotspots, and the largest improvements.
...
3. While they run, use `make lint` to identify the biggest regressions and improvements, then run any other narrow local checks you need.
